...
int main(int argc,char *argv[]) {
    ...
    int n,m;
    printf("input n m (n is dimension, m is point size) : ");
    scanf("%d %d",&n,&m);
    ...
    for ( int generation = 1 ; generation <= MAX_GENERATION ; generation++ ) {
        ...
        sprintf(filename,"result/g%d.vp",generation);
        ...
        sprintf(filename,"result/g%d.result",generation);
        fp = fopen(filename,"w");
        fprintf(fp,"now generation = %d tot f = %lf\n",generation,calculateNowGenesF(genes));
